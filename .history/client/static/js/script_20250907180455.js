...
async function fetchAndRenderPopularProducts() {
    ...
    try {
        ...
        totalProducts = featuredProducts.map(fp => fp.product); // Store the actual product data
...
async function fetchAndRenderPopularProducts() {
    ...
    try {
        const response = await fetch(`${BASE_URL}/api/featured-products/`); // Fetch from the new endpoint
        if (!response.ok) throw new Error('Network response was not ok');
        ...
        // Render the products into the carousel
        popTrack.innerHTML = featuredProducts.map(fp => {
            // The API response is nested, so we access `fp.product`
            const p = fp.product;
            const img = p.image ? `${BASE_URL}${p.image}` : "/assets/img/placeholder.png";
            const rating = p.rating ? '★'.repeat(Math.round(p.rating)) : '';
            const price = parseFloat(p.price).toFixed(2);
