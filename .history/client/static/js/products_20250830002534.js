...
function productCardHTML(p) {
  ...
  const imageUrl = p.image ? `${BASE_URL}${p.image}` : "../assets/products-img/default.png";
...
function openModal(id) {
  ...
  modalWeight.textContent = `${p.weight} kg` || 'N/A';
  ...
  modalHero.src = p.images && p.images.length > 0
    ? `${BASE_URL}${p.images[0]}`
    : p.image ? `${BASE_URL}${p.image}` : "../assets/products-img/default.png";
